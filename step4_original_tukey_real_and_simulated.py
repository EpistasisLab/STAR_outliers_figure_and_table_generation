...
outlier_indices = pd.read_csv("fake_data_true_outliers.txt", delimiter = "\t").to_numpy()
fake_data = pd.read_csv("fake_data.txt", delimiter = "\t").to_numpy()
prefix = "original_method_outlier_removal"
if not os.path.exists(prefix):
    os.mkdir(prefix)
if not os.path.exists(prefix + "_untransformed"):
    os.mkdir(prefix + "_untransformed")
for bound in (np.arange(10) + 90):
    dataset_names, TPR, FPR, precision = np.arange(len(fake_data.T)), [], [], []
    for x, ind, name in zip(fake_data.T, outlier_indices.T, dataset_names):
        ...
        ind_pred = W > cutoff
        TPR.append(np.sum(ind_pred[ind == True])/np.sum(ind == True))
        FPR.append(np.sum(ind_pred[ind == False])/np.sum(ind == False))
        if np.sum(ind_pred) == 0:
            precision.append(1)
        else:
            precision.append(np.sum(ind_pred[ind == True])/np.sum(ind_pred))
        ...
        if bound == 99:
            ...
            plt.hist([W[ind == False], np.concatenate([W[ind], W[ind]])], bins = 75, density = True,
                     stacked = True, color = ['blue', 'red'], label = ["data", "outliers"],
                     edgecolor = 'black')
            plt.plot(range0, smooth_TGH, "c-", linewidth = 6, label = "fitted tukey distribution")
    ...
    df.to_csv("simulated_efficacies/original_method_" + str(bound) + "_efficacy_.txt", sep = "\t", header = True, index = False)

prefix = "original_method_outlier_removal_real_data"
if not os.path.exists(prefix):
    os.mkdir(prefix)
if not os.path.exists(prefix + "_untransformed"):
    os.mkdir(prefix + "_untransformed")
...
used_names, removed_sets = [], []
for bound in (np.arange(10) + 90):
    removed = []
    for x, name in zip(real_data, dataset_names):
        ...
        if len(np.unique(x)) >= 10:
            ...
            x_outliers = x[W > cutoff]
            ind_pred = W > cutoff
            removed.append(len(x_outliers)/len(x))
            ...
            if bound == 99:
                ...
                range0 = np.linspace(xlims[0] - delta, xlims[1] + delta, np.max([250, int(len(W)/300)]))
                smooth_TGH = smooth(fitted_TGH, bw_method =  bw_coef)(range0)
                plt.hist(W, bins = 75, density = True, edgecolor = 'black')
                plt.plot(range0, smooth_TGH, "c-", linewidth = 6, label = "fitted tukey distribution")
                plt.plot([cutoff, cutoff], [0, 1], "k-", label = "predicted outlier threshold")
                plt.legend()
                plt.title("dataset" + str(name) + "_bound" + str(bound))
                ...
                plt.hist(x, bins = 75, density = True, color = 'blue', edgecolor = 'black')
                plt.savefig(prefix + "_untransformed/dataset" + str(name) + "_bound" + str(bound) + ".png")
                plt.clf()

    df = pd.DataFrame(np.array([used_names, removed]).T)
    df.columns = ["dataset", "fraction removed"]
    df.to_csv("real_efficacies/original_method_" + str(bound) + "_real_efficacy_.txt", sep = "\t", header = True, index = False)
